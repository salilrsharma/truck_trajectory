...
%         Truck_lane_change_data=[Truck_lane_change_data;l,lane_change_duration, lag_gap, speed_at_merge, merge_location];
    ...
    if ismember(k,false_id)
        if k==14
            lane_change_initiation = 5;
            lane_change_completion = 94;
        elseif k==24
            lane_change_initiation = 59;
            lane_change_completion = 138;
        elseif k==25
            lane_change_initiation = 93;
            lane_change_completion = 187;
        else
            lane_change_initiation = 50;
            lane_change_completion = 146;
        end
    else
        k;
    end
...
%% Plot trajectory data on nearside lane
% figure(10);
% for i =1: length(trajectories)
%     temp_lane = trajectories(i).lanes;
%     %nearside lane representation
%     desired_lane_id = find(temp_lane == 1);
%     if isempty(desired_lane_id)
%         continue;
%     else
%         hold on;
%         x_values = trajectories(i).x;
%         t_values = trajectories(i).t;
%         if ismember(i,counter)
%             color_line = 'r';
%         else
%             if trajectories(i).l >=12
%                 color_line = 'g';
%             else
%                 color_line = 'b';
%             end
%         end
%         plot(t_values(desired_lane_id), x_values(desired_lane_id), color_line);
%     end
% end